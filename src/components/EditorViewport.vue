...
const props = defineProps<{ document?: SourceDocument; catalog: ResourceCatalog; soldier?: SoldierAssets; options: ViewOptions; selectedId?: number; revision: number }>();
const emit = defineEmits<{ select: [number]; move: [SourceNode, string, [number, number, number]] }>();
const host = ref<HTMLElement>(); const fps = ref(0); const dynamicOccupants = ref(0); let controller: SceneController | undefined;
...
async function refresh() { if (controller && props.document) { await controller.setDocument(props.document, props.catalog, props.soldier, props.options); select(); } }
function select() { controller?.select(props.selectedId === undefined ? undefined : props.document?.nodes[props.selectedId]); }
watch(() => [props.document, props.revision, props.soldier, props.options.showBroken, props.options.showOccupants, props.options.showBounds, props.options.showShields], refresh);
watch(() => props.selectedId, select);
defineExpose({ reset: () => controller?.resetCamera(), top: () => controller?.topView(), side: () => controller?.sideView() });
</script>
<template>
  <div ref="host" class="viewport-host">
    <div class="viewport-help">左键旋转 · 右键平移 · 滚轮缩放 · 单击模型选择 · 拖动箭头修改位置</div>
    <div class="fps-badge" :class="{ slow: fps > 0 && fps < 25 }">{{ fps || '—' }} FPS · {{ dynamicOccupants }} 动态乘员</div>
    <div class="view-buttons"><button @click="controller?.resetCamera()">透视</button><button @click="controller?.topView()">顶视</button><button @click="controller?.sideView()">侧视</button></div>
  </div>
</template>
